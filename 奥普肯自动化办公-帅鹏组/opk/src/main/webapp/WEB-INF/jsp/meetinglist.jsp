	...
	<div class="rightinfo">
		...
		<div class="tools">
			...
			<ul class="toolbar">
				...
				<li style="padding: 0 5px"><form method="post" action="searchmeeting.action?type=1">
			...
			<tbody>
				<c:forEach items="${list}" var="list">
					<tr>
						...
						<td>${list.content}</td>
						<!-- 根据审批状态设置可执行操作 -->
						<td><c:if test="${list.approveStateId<4}">
								<c:if test="${list.approveStateId==2&&current_user.jobId==3}">
									<a
										href="agree.action?mId=${list.mId}&approveStateId=${list.approveStateId}"
										class="tablelink">通过</a>&nbsp;|<a
										href="reject.action?mId=${list.mId}&approveStateId=${list.approveStateId}"
										class="tablelink"> 驳回</a>
								</c:if>
								<c:if test="${list.approveStateId==3&&current_user.jobId==4}">
									<a
										href="agree.action?mId=${list.mId}&approveStateId=${list.approveStateId}"
										class="tablelink">通过</a>&nbsp;|<a
										href="reject.action?mId=${list.mId}&approveStateId=${list.approveStateId}"
								...
								test="${list.approveStateId>4&&list.approveStateId<8||list.content!=null}">
								<a href="deletemeeting.action?mId=${list.mId}" class="tablelink">
									删除</a>
							</c:if> <c:if test="${list.approveStateId==4}">
								<a
									href="complete.action?mId=${list.mId}&approveStateId=${list.approveStateId}"
									class="tablelink"> 已召开</a>
							</c:if></td>
					</tr>
				</c:forEach>
			</tbody>
